...
exports.default = {
    ...
    async autocomplete(interaction) {
        ...
        let arr = [];
        if (f.name === 'type')
            arr = cfg?.loaTypes || ['Full LOA'];
        else if (f.name === 'department')
            arr = cfg?.departments || ['General'];
        const filtered = arr.filter((c) => c.toLowerCase().includes(f.value.toLowerCase()));
    ...
    async execute(interaction) {
        ...
        const recent = await LOA_1.default.findOne({ userId: interaction.user.id, guildId: interaction.guildId, createdAt: { $gte: new Date(Date.now() - cfg.cooldownHours * 3600000) } }).sort({ createdAt: -1 });
        if (recent) {
