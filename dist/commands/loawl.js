    ...
    async execute(interaction) {
        ...
        if (sub === 'add') {
            if (cfg.whitelistRoles.includes(role.id))
                return interaction.reply({ content: 'Already whitelisted.', ephemeral: true });
            cfg.whitelistRoles.push(role.id);
            await cfg.save();
            return interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(0x57F287).setTitle('✅ Added').setDescription('<@&' + role.id + '> added to whitelist.')], ephemeral: true });
        }
        else if (sub === 'remove') {
            const idx = cfg.whitelistRoles.indexOf(role.id);
            if (idx === -1)
                return interaction.reply({ content: 'Not in whitelist.', ephemeral: true });
            cfg.whitelistRoles.splice(idx, 1);
            await cfg.save();
            return interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(0xED4245).setTitle('❌ Removed').setDescription('<@&' + role.id + '> removed from whitelist.')], ephemeral: true });
        }
        else {
            if (!cfg.whitelistRoles.length)
                return interaction.reply({ content: 'Whitelist empty.', ephemeral: true });
            const list = cfg.whitelistRoles.map(id => '<@&' + id + '>').join(', ');
            return interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(0xCFB87C).setTitle('🛡️ Whitelisted Roles').setDescription(list)] });
        }
    }
};
