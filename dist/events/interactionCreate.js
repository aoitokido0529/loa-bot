...
async function submitLOA(interaction, type, start, end, dept, reason) {
    ...
    if (!config)
        return interaction.reply({ content: '❌ Not configured.', ephemeral: true });
    if (end <= start)
        return interaction.reply({ content: '❌ End must be after start.', ephemeral: true });
    const days = Math.ceil((end.getTime() - start.getTime()) / 86400000);
    if (days > config.maxLoaDuration)
    ...
    await loa.save();
    const embed = new discord_js_1.EmbedBuilder().setColor(0x5865F2).setTitle('📝 LOA Submitted')
    ...
    if (config.logChannel) {
        ...
        if (ch?.isTextBased()) {
            const logEmbed = new discord_js_1.EmbedBuilder().setColor(0xFEE75C).setTitle('⏳ New LOA')
...
function createLOAEmbed(loa) {
    const colors = { pending: 0xFEE75C, approved: 0x57F287, denied: 0xED4245, cancelled: 0x95A5A6, expired: 0x95A5A6 };
    const emojis = { pending: '⏳', approved: '✅', denied: '❌', cancelled: '🚫', expired: '⏰' };
    return {
