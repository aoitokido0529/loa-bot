            ...
            sub.setName('config')
                ...
                .addStringOption(o => o.setName('setting').setDescription('Setting to modify')
                    .addChoices(
                        ...
                        { name: '🏷️ LOA Role', value: 'loaRole' },
                        { name: '📅 Max Duration', value: 'maxLoaDuration' },
                        { name: '⏰ Cooldown Hours', value: 'cooldownHours' }
                    ))
    ...
    async execute(interaction: any) {
        ...
        const resolveRole = async (input: string): Promise<string | null> => {
            ...
            const role = interaction.guild.roles.cache.find(
                (r: any) => r.name.toLowerCase() === input.toLowerCase()
            );
            return role ? role.id : null;
        };

        // Helper to resolve a comma-separated list of roles
        const resolveRoles = async (input: string): Promise<string[] | null> => {
            const parts = input.split(',').map(p => p.trim()).filter(Boolean);
            const ids: string[] = [];
            for (const part of parts) {
                const id = await resolveRole(part);
                if (!id) return null; // one failed, abort
                ids.push(id);
            }
            return ids;
        };
            ...
            case 'logChannel': {
                ...
                if (!m) {
                    ...
                    const ch = interaction.guild.channels.cache.find(
                        (c: any) => c.name.toLowerCase() === value.toLowerCase() && c.isTextBased()
                    );
                    if (!ch) {
        ...
        await interaction.reply({
            embeds: [new EmbedBuilder().setColor(0x57F287).setTitle('✅ Updated').setDescription(`**${setting}** → **${value}**`).setTimestamp()],
            ephemeral: true
        });
    }
};