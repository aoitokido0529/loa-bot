    ...
    async execute(interaction: any) {
        ...
        if (sub === 'check') {
            ...
            if (t.id !== interaction.user.id) {
                ...
                if (!isStaff) return interaction.reply({ content: '❌ Own cooldown only.', ephemeral: true });
            }
            const recent = await LOA.findOne({ userId: t.id, guildId: interaction.guildId, createdAt: { $gte: new Date(Date.now() - cfg.cooldownHours * 3600000) } }).sort({ createdAt: -1 });
            const embed = new EmbedBuilder().setTitle('Cooldown').addFields([{ name: 'User', value: '<@' + t.id + '>', inline: true }, { name: 'Cooldown', value: cfg.cooldownHours + 'h', inline: true }]);
            if (recent) {
        ...
        const nonApproved = await LOA.findOne({ userId: target.id, guildId: interaction.guildId, status: { $ne: 'approved' }, createdAt: { $gte: new Date(Date.now() - cfg.cooldownHours * 3600000) } }).sort({ createdAt: -1 });
        if (!nonApproved) {
            const approved = await LOA.findOne({ userId: target.id, guildId: interaction.guildId, status: 'approved', createdAt: { $gte: new Date(Date.now() - cfg.cooldownHours * 3600000) } });
            if (approved) return interaction.reply({ content: '⚠️ Active LOA #' + approved.loaId + '. Cancel it first.', ephemeral: true });
            return interaction.reply({ content: '✅ Not on cooldown.', ephemeral: true });
        }
        await LOA.deleteOne({ _id: nonApproved._id });
        return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x57F287).setTitle('✅ Cooldown Cleared').setDescription('Removed #' + nonApproved.loaId + ' (' + nonApproved.status + ') for <@' + target.id + '>.')] });
    }
};